...
void hashmap::put(int key, int value) {
  int hashBucket;
  hashBucket = hashFunction(key) % nBuckets;
  key_val_pair *curr = buckets[hashBucket];
  ...
  key_val_pair* add = new key_val_pair;
  nElems++;
  add -> key = key; 
  add -> value = value;
  add -> next = nullptr;
...
int hashmap::get(int key) const {
  ...
  throw runtime_error("Error: Key is not in Hashtable");
}
